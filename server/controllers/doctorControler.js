...
exports.getDoctorCalendarById = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al obtener el calendario del médico" });
  }
};
...
exports.getDoctorBySpecialityAndCenter = async (req, res) => {
  ...
  try {
    ...
    console.error(
      "Error al obtener los médicos por especialidad y centro:",
      error,
    );
    res
      .status(500)
      .json({
        error: "Error al obtener los médicos por especialidad y centro",
      });
  }
};
...
exports.getPacientesSinAtender = async (req, res) => {
  ...
  try {
    ...
    const agendasSinAtender = await Agenda.find({
      _id: { $in: doctor.agendaId },
      atencion: false,
    ...
    res
      .status(500)
      .json({ error: "Error al obtener los pacientes sin atender" });
  }
};

exports.patientAttended = async (req, res) => {
  const { patient } = req.body;
  try {
    // Assuming patient has email
    const agenda = await Agenda.findOne({
      email_paciente: patient.email,
      atencion: false,
    });
    if (agenda) {
      agenda.atencion = true;
      await agenda.save();
    }
    res.json({ message: "Patient marked as attended" });
  } catch (error) {
    console.error("Error updating patient attended:", error);
...
exports.updateAvailability = async (req, res) => {
  ...
  try {
    ...
    console.error(
      "Error al actualizar la disponibilidad y generar el calendario del médico:",
      error,
    );
    res
      .status(500)
...
const generateCalendar = (availability) => {
  ...
  while (currentDate <= endDate) {
    ...
    const availabilityOfDay = availability.find(
